...
# Function to print rainbow text [cite: 129]
print_rainbow() {
    local text="$1"
    local length=${#text}
...
directories=(
    ...
    /etc/zivpn # Menyesuaikan path untuk menu/api ZiVPN [cite: 133, 134]
)
...
# === LOKASI SISIPAN: DOS2UNIX MASSAL === 
apt update -y
apt install -y dos2unix wget curl zip unzip p7zip-full jq [cite: 153, 154]

# Jalankan sisa instalasi paket asli anda 
packages=(
    ...
    rsyslog zlib1g-dev libssl-dev libsqlite3-dev sed dirmngr libxml-parser-perl build-essential
    ...
    tmux python3 python3-pip lsb-release gawk libncursesw5-dev libgdbm-dev tk-dev libffi-dev libbz2-dev checkinstall
...
# === LOGIKA INSTALASI NGINX & HAPROXY ASLI === [cite: 168, 172, 184]
...
if [[ $os_id == "ubuntu" ]]; then
    ...
    apt update -y && apt install -y nginx
    apt install -y haproxy [cite: 177]
fi

# === EKSTRAKSI MENU & PERBAIKAN PATH ZIVPN === 
...
# SISIPAN PERBAIKAN: dos2unix massal agar menu tidak error 
dos2unix * >/dev/null 2>&1
chmod +x * >/dev/null 2>&1
mv * /usr/bin >/dev/null 2>&1

# SISIPAN: Instalasi ZiVPN Core 
...
# SISIPAN: Sinkronisasi file apikey agar tidak cat error 
mkdir -p /etc/zivpn
...
# === LANJUTKAN DENGAN KODE ASLI ANDA (VPN CONFIGS) === [cite: 215, 224, 230]
...
# === LOGIKA SERVICE XRAY ASLI === [cite: 247]
create_service() {
...
rm -rf /etc/menu && rm -f /root/rmck [cite: 256]
clear
