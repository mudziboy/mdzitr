...
EOF
    ...
    echo -e "${blue}    │   ${green}├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤   ${blue} │${neutral}"
    echo -e "${blue}    │   ${green}┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘${blue} │${neutral}"
    echo -e "${blue}    │   ${yellow}Copyright${reset} (C)${gray} https://t.me/rahmarie${blue} │${neutral}"
    echo -e "${blue}    └───────────────────────────────────────────────┘${neutral}"
    echo -e "${blue}    ────────────────────────────────────────────────${neutral}"
...
echo -e "${yellow}Downloading project.zip...${neutral}"
if wget -O menu.zip "$url"; then
    echo -e "${green}Download complete.${neutral}"
else
    echo -e "${red}Failed to download project.zip${neutral}"
    exit 1
fi

echo -e "${yellow}Extracting project.zip...${neutral}"
7z e -pLebakSari menu.zip >/dev/null 2>&1
if [ $? -ne 0 ]; then
    echo -e "${red}Failed to extract project.zip${neutral}"
    exit 1
fi
...
mv * /usr/bin/
cd ..
rm -rf /etc/menu menu.zip
echo -e "${green}Project installed successfully.${neutral}"
...
wget -q -O /root/.bot/bot.zip https://raw.githubusercontent.com/joytun21/xrayz/raw/main/bot.zip
